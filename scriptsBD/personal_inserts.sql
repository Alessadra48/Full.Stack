...
SELECT nombre_jefe_depto, Ciudad AS 'JEFES' FROM departamentos  WHERE Ciudad = 'Ciudad Real'; 
...
SELECT nombre, sal_emp, comision_emp, (sal_emp + comision_emp) as salario_total FROM empleados HAVING comision_emp > 1000; -- where
-- 15. Obtener un listado similar al anterior, pero de aquellos empleados que NO tienen comisión.
SELECT nombre FROM empleados WHERE comision_emp IS NULL OR comision_emp = 0; 
...
SELECT nombre, sal_emp, comision_emp FROM empleados WHERE nombre NOT LIKE '%MA%'; 
...
SELECT MAX(sal_emp) AS Salario_Maximo, MIN(sal_emp) AS Salario_Minimo, (MAX(sal_emp) - MIN(sal_emp)) AS Diferencia_Salarial FROM empleados;
...
SELECT nombre, nombre_depto, nombre_jefe_depto FROM departamentos d LEFT JOIN empleados e ON e.id_depto = d.id_depto ;
